...
pub fn read(self: SX_Distributor, db: *DB) !void {
    ...
    if (!DB.is_valid_id(id)) {
        log.warn("Skipping Distributor {s} (invalid ID)", .{ id });
        return;
    }
...
pub fn write_dirty(allocator: std.mem.Allocator, db: *DB, root: *std.fs.Dir, filenames: *paths.StringHashSet) !void {
    ...
    for (0..db.dists.len, db.dists.items(.id), db.dists.items(.modified_timestamp_ms)) |i, id, modified_ts| {
        ...
        log.info("Writing dist{s}{s} (modified {" ++ DTO.fmt_sql_ms ++ "})", .{ std.fs.path.sep_str, dest_path, modified_dto });
