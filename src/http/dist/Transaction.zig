...
created_idx: ?Distributor.Index = null,
    ...
    const idx = self.idx orelse {
        ...
        for (self.relations.values()) |relation| {
            if (relation.other.future_opt()) |other| {
                ...
                try Distributor.Relation.set_ordering(db, idx, relation_idx, 0xFFFF);
            }
        }
...
pub fn get_post_prefix(db: *const DB, maybe_idx: ?Distributor.Index) ![]const u8 {
    if (maybe_idx) |idx| {
        const id = Distributor.get_id(db, idx);
        return try http.tprint("/dist:{}", .{ http.fmtForUrl(id) });
    }
    return "/dist";
}
